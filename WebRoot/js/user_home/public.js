...
/**
 * 获取可见分页
 * @param viewpagecount 可见分页总数
 * @param currentPage 当前页
 * @param totalpage 总页数
 * @returns
 */
function getPageIndex(viewpagecount,currentPage,totalpage){
	var startpage = currentPage-(viewpagecount%2==0? viewpagecount/2-1 : viewpagecount/2);
	var endpage = currentPage+viewpagecount/2;
	if(startpage<1){
		startpage = 1;
...
/**
 * 用于显示分页链接
 * @param strUrl 需要分页的链接
 * @param currentNumber 当前页
 * @param totalNumer 总页数
 * @returns {String}返回html分页代码
 */
function pagerUtil(strUrl,currentNumber,totalNumer){
	var pager = "<center>";
	if(currentNumber>1){
		pager += " <a href=javascript:turnPage('"+strUrl+0+"')>首页</a> ";
		pager += " <a href=javascript:turnPage('"+strUrl+(currentNumber-2)*10+"')>上一页</a> ";
	}else{
		pager += " 首页 ";
	}
	var obj = getPageIndex(10,currentNumber,totalNumer);
	for(var i=obj[0];i<=obj[1];i++){
		if(currentNumber == i){
			pager += " <font color=red>"+i+"</font> ";
		}else{
			pager += " <a href=javascript:turnPage('"+strUrl+(i-1)*10+"')>"+i+"</a> ";
		}
	}
	if(currentNumber<totalNumer){
		pager += " <a href=javascript:turnPage('"+strUrl+(currentNumber)*10+"')>下一页</a> ";
		pager += " <a href=javascript:turnPage('"+strUrl+(totalNumer-1)*10+"')>尾页</a> ";
	}else{
		pager += " 尾页  ";
	}	
	pager += "</center>";
